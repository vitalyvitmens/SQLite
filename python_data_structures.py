...
# TODO: Linked List (Связанный список)
#  Связанный список — это последовательность узлов, где каждый узел хранит свои данные и ссылку на следующий узел.
#  Один узел связан с другим, образуя то, что можно представить как связанную цепочку:
#  см. фото Linked List.jpg
#  первый узел называется головным, и он используется в качестве отправной точки для любой итерации по списку.
#  Последний узел должен иметь ссылку, указывающую на None, чтобы определить конец списка.
#  В отличие от стеков и очередей, вы можете вставлять и удалять узлы в любом месте связанного списка
#  (аналогично стандартному списку).
#  Applications (Приложения):
#  Связанные списки полезны, когда ваши данные связаны. Например, когда вам нужна функция отмены/возврата,
#  узлы могут представлять состояние со ссылками на предыдущее и следующее состояния.
#  Другим примером может быть музыкальный плейлист, где каждый клип связан со следующим.
#  Связанные списки также можно использовать для создания других структур данных, таких как стек, очереди и графы.

# TODO: Связанный список в Python
#  Каждый узел будет включать данные и ссылку на следующий узел. Начнем с создания класса Node:
# class Node:
#     def __init__(self, data, next):
#         self.data = data
#         self.next = next
# TODO: Теперь мы можем создать класс LinkedList с соответствующими методами:
#  Метод add_at_front() добавляет новый Node в качестве головы списка и связывает с ним предыдущую голову.
#  Метод add_at_end() выполняет итерацию до конца списка, используя цикл while,
#  и добавляет новый узел в качестве ссылки на последний узел.
...
# TODO: ЗАДАЧА: Связанный список
#  Вы делаете музыкальный проигрыватель, который позволяет вам создать список воспроизведения треков.
#  Данный код определяет классы Player и Track, где Player представляет собой связанный список,
#  объединяющий в цепочку объекты Track.
#  Код берет несколько треков из пользовательского ввода и добавляет их в список воспроизведения.
#  Вам нужно перебрать связанный список и вывести все треки в плейлисте в порядке воспроизведения.
#  Используйте цикл while для перебора связанного списка.
# class Track:
...
# TODO: Graph (Графы) используются для представления многих реальных приложений, таких как сети,
#  транспортные пути города и связи в социальных сетях. Граф представляет собой набор связанных узлов,
#  где каждый узел называется вершиной, а соединение между двумя из них называется ребром.
#  Вот пример графа: Это может представлять, например, связи в социальной сети,
#  где каждая вершина представляет человека, а ребра представляют связи.
#  см. фото Graph.png

# TODO: Граф можно представить с помощью квадратной матрицы, где каждый элемент представляет ребра:
#  0 указывает на отсутствие ребра, а 1 указывает на ребро. Строки и столбцы представляют вершины.
#  Например:
#  0 1 1
#  1 0 0
#  1 0 0
...
#  Матрица такого типа называется матрицей смежности, потому что она показывает,
#  являются ли соответствующие вершины смежными или нет.

# TODO: Давайте реализуем класс Graph:
#  Мы храним матрицу в двумерном списке с именем adj.
...
#  Точно так же метод remove_edge() устанавливает значения в 0.
# class Graph():
...
# TODO: ЗАДАЧА: Графы
#  Вы создаете социальную сеть под названием X. Связи между пользователями сохраняются в виде графа.
#  Данный код объявляет класс X с его методом add_friend() и создает несколько соединений для 5 пользователей.
#  Вам нужно взять число на вход и вывести количество подключений соответствующего пользователя.
#  Соединения каждого пользователя хранятся в матрице adj.
# class X:
#     def __init__(self, size):
#         self.adj = [[0] * size for i in range(size)]
#         self.size = size
...
#                 a += 1
#
# print(f'Вы ввели: {n} => {a}', end='')
# x.display()

# TODO: ЗАДАЧА: Сбалансированные скобки
#  Скобки сбалансированы, если все открывающие скобки имеют соответствующие закрывающие скобки.
#  Учитывая выражение в качестве входных данных, нам нужно выяснить, сбалансированы ли скобки или нет.
#  Например, "(x+y)*(z-2*(6))" - сбалансировано, а "7-(3(2*9))4) (1" - не сбалансировано.
#  Задача может быть решена с помощью стек.
#  Поместите каждую открывающую скобку в стек и извлеките последнюю вставленную открывающую скобку всякий раз,
#  когда встречается закрывающая скобка. Если закрывающая скобка не соответствует открывающей скобке, то
#  остановитесь и скажите, что скобки не сбалансированы.
#  Кроме того, после проверки всех круглых скобок нам нужно проверить, что стек пуст — если он не пуст,
#  значит круглые скобки не сбалансированы. Реализуйте функцию balance(), чтобы она возвращала True,
#  если скобки в данном выражении сбалансированы, и False, если нет.
#  Пример ввода: (a( ) eee))
#  Пример вывода: False
#  Вы можете использовать простой список для стека.
#  Используйте list.insert(0, item) для добавления в стек и list.pop(0) для извлечения верхнего элемента.
#  Вы можете получить доступ к верхнему элементу стека, используя list[0] .
