...
# TODO: Metacharacters - Еще несколько метасимволов * + ? { и }.
#  Они определяют количество повторений.
#  Метасимвол * означает «ноль или более повторений предыдущей вещи».
#  Он пытается соответствовать как можно большему количеству повторений.
#  «Предыдущая вещь» может быть отдельным символом, классом или группой символов в круглых скобках.
# import re
#
# """
# В примере сопоставляются строки, начинающиеся с «egg» и заканчивающиеся нулем или более "spam".
# """
# pattern = r"egg(spam)*"
...
# TODO: Метасимвол + очень похож на * , за исключением того, что он означает
#  «одно или несколько повторений», а не «ноль или более повторений».
#  Подводя итог:
...
# TODO: Метасимвол ? означает «ноль или одно повторение».
# import re
#
# pattern = r"ice(-)?cream"
#
...
# TODO: Фигурные скобки можно использовать для представления количества повторений между двумя числами.
#  Регулярное выражение {x,y} означает «между x и y повторениями чего-то».
#  Следовательно, {0,1} — это то же самое, что и ?.
#  Если первое число отсутствует, оно считается равным нулю.
#  Если второе число отсутствует, оно считается бесконечным.
# import re
#
# """
# "9{1,3}$" соответствует строке, содержащей от 1 до 3 девяток.
# """
# pattern = r"9{1,3}$"
#
...
#
# """
# Создайте программу, которая принимает пароль в качестве входных данных и возвращает "Password created", если:
# - содержит хотя бы один символ верхнего регистра
# - содержит хотя бы одну цифру
# Программа должна выводить "Wrong format", если вышеуказанные требования не выполняются.
# Образец ввода:
# Hal44gb8
# Образец вывода:
# Password created
...
# TODO: Groups - Группу можно создать, заключив часть регулярного выражения в круглые скобки.
#  Это означает, что группа может быть задана в качестве аргумента метасимволов, таких как * и ?.
#  (spam) представляет группу в примере шаблона, показанном ниже:
# import re
#
# pattern = r"egg(spam)*"
#
...
# TODO: Доступ к содержимому групп в match можно получить с помощью групповой функции.
#  Вызов group(0) или group() возвращает полное совпадение.
#  Вызов group(n), где n больше 0, возвращает n-ную группу слева.
#  Метод groups() возвращает все группы начиная с 1.
#  Как видно из примера ниже, группы могут быть вложенными:
# import re
#
# pattern = r"a(bc)(de)(f(g)h)i(jk)"
#
...
# TODO: Существует несколько видов специальных групп.
#  Двумя полезными являются именованные группы и группы без захвата.
#  Именованные группы имеют формат (?P<name>...), где name — это имя группы, а ... — ее содержимое.
#  Они ведут себя точно так же, как обычные группы, за исключением того,
#  что к ним можно получить доступ по группе (имени) в дополнение к ее номеру.
#  Незахватывающие группы имеют формат (?:...). Они недоступны групповым методом,
#  поэтому их можно добавить в существующее регулярное выражение, не нарушая нумерацию.
# import re
#
...
# TODO: Еще один важный метасимвол | .
#  Это означает «или», поэтому красный|синий соответствует либо «красному», либо «синему».
# import re
#
# pattern = r"gr(a|e)y"
#
...
# TODO: Special Sequences - Существуют различные специальные последовательности,
#  которые вы можете использовать в регулярных выражениях.
#  Они записываются в виде обратной косой черты, за которой следует другой символ.
#  Одной из полезных специальных последовательностей является обратная косая черта и число от 1 до 99,
#  например, \1 или \17. Это соответствует выражению группы этого числа.
#  Обратите внимание, что " (.+) \1 " не совпадает с " (.+) (.+) ", потому что \1 относится к
#  подвыражению первой группы, которое является самим совпадающим выражением, а не шаблоном регулярного выражения.
# import re
#
# pattern = r"(.+) \1"
#
...
# TODO: Более полезными специальными последовательностями являются \d , \s и \w
#  Они соответствуют цифрам, пробелам и символам слова соответственно.
#  В режиме ASCII они эквивалентны [0-9], [\t\n\r\f\v] и [a-zA-Z0-9_].
#  В режиме Unicode они также соответствуют некоторым другим символам.
#  Например, \w соответствует буквам с диакритическими знаками.
#  Версии этих специальных последовательностей с буквами в верхнем регистре — \D , \S и \W — означают
#  противоположность версиям в нижнем регистре. Например, \D соответствует всему, что не является цифрой.
#  (\D+\d) соответствует одной или нескольким нецифрам, за которыми следует цифра.
# import re
#
# pattern = r"(\D+\d)"
#
...
# TODO: Дополнительные специальные последовательности: \A , \Z и \b .
#  Последовательности \A и \Z соответствуют началу и концу строки соответственно.
#  Последовательность \b соответствует пустой строке между символами \w и \W или символам \w и началу или концу строки.
#  Неформально он представляет собой границу между словами.
#  Последовательность \B соответствует пустой строке в любом другом месте.
#  \b(кошка)\b " в основном соответствует слову "кошка", окруженному границами слов.
# import re
#
# pattern = r"\b(cat)\b"
#
...
#
# text = input()
# math = re.findall(r"#\w+", text)
# if math:
#     print("\n".join(math))

# TODO: Email Extraction - Чтобы продемонстрировать пример использования регулярных выражений,
#  давайте создадим программу для извлечения адресов электронной почты из строки.
#  Предположим, у нас есть текст, содержащий адрес электронной почты:
#  str = "Please contact info@sololearn.com for assistance"
#  Наша цель — извлечь подстроку «info@sololearn.com».
#  Основной адрес электронной почты состоит из слова и может включать точки или тире.
#  Затем следует знак @ и доменное имя (имя, точка и суффикс доменного имени).
#  Это основа для построения нашего регулярного выражения:
#  pattern = r"([\w\.-]+)@([\w\.-]+)(\.[\w\.]+)"
#  [\w\.-]+ соответствует одному или нескольким символам слова, точкам или тире.
#  Приведенное выше регулярное выражение говорит, что строка должна содержать слово (с разрешенными точками и тире),
#  за которым следует знак @, затем еще одно подобное слово, затем точка и еще одно слово.
#  Наше регулярное выражение содержит три группы:
#  1 — первая часть адреса электронной почты.
#  2 - доменное имя без суффикса.
#  3 - суффикс домена.
#  Если строка содержит несколько адресов электронной почты, мы можем использовать метод re.findall вместо re.search
#  для извлечения всех адресов электронной почты.

# import re
#
# pattern = r"([\w\.-]+)@([\w\.-]+)(\.[\w\.]+)"
# str = "Please contact otm_@tut.by for assistance"
#
# match = re.search(pattern, str)
# if match:
#     print(match.group())

# TODO: Вам дается числовой ввод, и вам нужно проверить, является ли он действительным номером телефона.
#  Действительный номер телефона состоит ровно из 8 цифр и начинается с 1 , 8 или 9 .
#  Выведите «Valid», если номер правильный, и «Invalid», если нет.
#  Пример ввода:
#  81239870
#  Пример вывода:
#  Valid
...
# TODO: The Zen of Python - Написание программ, которые на самом деле делают то, что от них требуется,
#  — это лишь один из компонентов хорошего программиста на Python.
#  Также важно писать чистый код, который легко понять, даже через несколько недель после того, как вы его написали.
#  Один из способов сделать это — следовать Zen of Python, несколько насмешливому набору принципов,
#  который служит руководством по программированию на языке Pythoneer.
#  Используйте следующий код для доступа к Zen of Python.
#  import this
...
#  Ошибки никогда не должны проходить бесшумно. Если явно не заглушить.
...
# TODO: Некоторые строки в Zen of Python могут нуждаться в дополнительных пояснениях:
#  Явное лучше, чем неявное: лучше всего четко указать, что делает ваш код. Вот почему добавление числовой строки к
#  целому числу требует явного преобразования, а не происходит за кулисами, как это происходит в других языках.
#  Flat лучше, чем вложенный: следует избегать сильно вложенных структур (списков списков, списков и т. д. и т. д.).
#  Ошибки никогда не должны проходить молча: в общем, когда возникает ошибка, вы должны выводить какое-то сообщение
#  об ошибке, а не игнорировать его. В Zen of Python 20 принципов, но всего 19 строк текста.
#  20-й принцип зависит от мнения, но наша интерпретация такова, что пустая строка означает «Использовать пробелы».
#  Строка «Должна быть одна — и желательно только один — очевидный способ сделать это» отсылает и противоречит
#  философии языка Perl, согласно которой должно быть более одного способа сделать это.

# TODO: PEP - Предложения по улучшению Python (PEP) — это предложения по улучшению языка,
#  сделанные опытными разработчиками Python.
#  PEP 8 — это руководство по стилю написания удобочитаемого кода.
#  Он содержит ряд указаний относительно имен переменных, которые приведены здесь:
#  - Модули должны иметь короткие имена, состоящие только из строчных букв;
#  - имена классов должны быть в стиле CapWords;
#  - имена большинства переменных и функций должны быть в нижнем регистре_с_подчеркиванием;
#  - константы (переменные, значения которых никогда не меняются) должны быть CAPS_WITH_UNDERSCORES;
#  - имена, которые конфликтуют с ключевыми словами Python (например, «класс» или «если»),
#  должны иметь завершающее подчеркивание. PEP 8
#  также рекомендует использовать пробелы вокруг операторов и после запятых для повышения удобочитаемости.
#  Однако не следует злоупотреблять пробелами. Например, избегайте пробелов непосредственно внутри скобок любого типа.

# TODO: PEP 8 - Другие предложения PEP 8 включают следующее:
#  - строки не должны быть длиннее 80 символов;
#  - следует избегать "из импорта модуля *";
#  - в строке должно быть только одно выражение. Также предлагается использовать для отступов пробелы, а не табуляцию.
#  Однако в какой-то степени это вопрос личных предпочтений. Если вы используете пробелы, используйте только 4 в строке.
#  Важнее выбрать один и придерживаться его. Самый важный совет в PEP — игнорировать его, когда это имеет смысл.
#  Не беспокойтесь о том, чтобы следовать предложениям PEP, если это сделает ваш код менее читаемым;
#  несовместим с окружающим кодом; или нет обратной совместимости.
#  Однако, по большому счету, следование PEP 8 значительно улучшит качество вашего кода.
#  Некоторые другие известные PEP, которые охватывают стиль кода:
#  PEP 20 : Zen of Python
#  PEP 257 : Соглашения о стилях для строк документации.

# TODO: Function Arguments - Python позволяет иметь функцию с различным количеством аргументов .
...
# TODO: Default Values (именованные параметры) функции можно сделать необязательными, присвоив им значение по умолчанию.
#  Они должны идти после именованных параметров без значения по умолчанию.
#  В случае передачи аргумента, значение по умолчанию игнорируется.
...
# TODO: Function Arguments - **kwargs (расшифровывается как аргументы ключевого слова)
#  позволяет вам обрабатывать именованные аргументы, которые вы не определили заранее.
...
# """
# Дана функция, которая принимает 2 аргумента и возвращает их сумму.
# Но мы получаем ошибку, когда хотим суммировать более 2 чисел.
# Измените функцию и завершите код так, чтобы функция суммировала столько чисел, сколько введено.
# """
#
#
# def adder(x, *args):
...
# TODO: Tuple Unpacking - Распаковка кортежа позволяет присвоить каждый элемент итерируемого объекта
#  (часто кортежа) переменной.
#  Это также можно использовать для замены переменных, выполнив a, b = b, a , поскольку b, a в правой части
#  формирует кортеж (b, a), который затем распаковывается.
# numbers = (1, 2, 3)
# a, b, c = numbers
# print(a)
# print(b)
...
# TODO:
#  Переменная, перед которой стоит звездочка (*), принимает все значения из итерации, оставшиеся от других переменных.
...
# TODO: Ternary Operator - Условные выражения обеспечивают функциональность операторов if, используя меньше кода.
#  Ими не следует злоупотреблять, так как они могут легко ухудшить читабельность, но
#  они часто бывают полезны при назначении переменных.
#  Условные выражения также известны как приложения тернарного оператора.
#  Тернарный оператор проверяет условие и возвращает соответствующее значение.
#  В приведенном выше примере, поскольку условие истинно, b присваивается значение 1.
#  Если бы a было меньше 5, ему было бы присвоено значение 42.
...
# to_cash = int(input())
#
# money_left = balance - to_cash if balance >= to_cash >= 500 else "Error"
# print(money_left)

# TODO: else - Оператор else чаще всего используется вместе с оператором if, но он также может
#  следовать за циклом for или while, что придает ему другое значение.
#  При использовании цикла for или while код внутри него вызывается, если цикл завершается нормально
#  (когда оператор break не вызывает выхода из цикла).
#  Первый цикл for выполняется нормально, что приводит к печати «Unbroken 1».
#  Второй цикл завершается из-за break , поэтому оператор else не выполняется.
# for i in range(10):
#     if i == 999:
#         break
# else:
...
#  В этом случае код внутри него выполняется только в том случае, если в операторе try не возникает ошибки .
# try:
#     print(1)
...
# TODO: __main__
#  Большая часть кода Python представляет собой либо импортируемый модуль, либо скрипт, который что-то делает.
#  Однако иногда бывает полезно создать файл, который можно импортировать как модуль и запускать как скрипт.
#  Для этого поместите код скрипта внутри if __name__ == "__main__" .
#  Это гарантирует, что он не будет запущен, если файл будет импортирован.
#  Когда интерпретатор Python читает исходный файл, он выполняет весь код, найденный в файле.
#  Перед выполнением кода он определяет несколько специальных переменных.
#  Например, если интерпретатор Python запускает этот модуль (исходный файл) в качестве основной программы,
#  он устанавливает для специальной переменной __name__ значение "__main__".
#  Если этот файл импортируется из другого модуля, __name__ будет установлено на имя модуля.
# def function():
...
# TODO: Если мы сохраним код из нашего предыдущего примера в виде файла с именем sololearn.py,
#  мы сможем затем импортировать его в другой скрипт как модуль, используя имя sololearn.
#  import sololearn
#  sololearn.function()
#  Открой файл module_function.py и запусти его, что бы посмотреть как это работает

# TODO: some_script.py
#  По сути, мы создали собственный модуль под названием sololearn, а затем использовали его в другом скрипте.

# TODO: Major 3rd-Party Libraries - Основные сторонние библиотеки
#  Одна только стандартная библиотека Python содержит обширную функциональность.
#  Однако для некоторых задач требуется использование сторонних библиотек.
#  Некоторые основные сторонние библиотеки:
#  Django : наиболее часто используемая веб-инфраструктура, написанная на Python.
#  Django поддерживает веб-сайты, в том числе Instagram и Disqus.
#  Он имеет много полезных функций, а те функции, которых ему не хватает, покрываются пакетами расширений.
#  CherryPy и Flask также являются популярными веб-фреймворками.
#  Для сбора данных с веб-сайтов библиотека BeautifulSoup очень полезна и дает лучшие результаты,
#  чем создание собственного парсера с регулярными выражениями.
#  Хотя Python предлагает модули для программного доступа к веб-сайтам, такие как urllib,
#  они довольно громоздки в использовании.
#  Запросы сторонних библиотек значительно упрощают использование HTTP-запросов.

# TODO: Доступен ряд сторонних модулей, которые значительно упрощают выполнение научных и
#  математических вычислений с помощью Python.
#  Модуль matplotlib позволяет создавать графики на основе данных в Python.
#  Модуль NumPy позволяет использовать многомерные массивы, которые намного быстрее,
#  чем собственное решение Python для вложенных списков.
#  Он также содержит функции для выполнения математических операций, таких как матричные преобразования массивов.
#  Библиотека SciPy содержит многочисленные расширения функциональности NumPy .
#  Python также можно использовать для разработки игр .
#  Обычно он используется в качестве языка сценариев для игр,
#  написанных на других языках, но может использоваться и для создания игр сам по себе.
#  Для 3D-игр можно использовать библиотеку Panda3D . Для 2D-игр вы можете использовать pygame .


# TODO: Packaging - В Python термин «упаковка» относится к размещению написанных вами модулей в стандартном формате,
#  чтобы другие программисты могли легко их установить и использовать.
#  Это предполагает использование модулей setuptools и distutils .
#  Первым шагом в упаковке является правильная организация существующих файлов.
#  Поместите все файлы, которые вы хотите поместить в библиотеку, в один и тот же родительский каталог.
#  Этот каталог также должен содержать файл с именем __init__.py ,
#  который может быть пустым, но должен присутствовать в каталоге.
#  Этот каталог переходит в другой каталог, содержащий readme и лицензию, а также важный файл с именем setup.py.
#  SoloLearn/
#    LICENSE.txt
#    README.txt
#    setup.py
...
#       sololearn.py
#       sololearn2.py
#  Вы можете разместить столько файлов скриптов в каталоге, сколько вам нужно.
#  Следующим шагом в упаковке является написание файла setup.py .
#  Он содержит информацию, необходимую для сборки пакета, чтобы его можно было загрузить в PyPI и
#  установить с помощью pip (имя, версия и т. д.).
...
# )
# TODO: После создания файла setup.py загрузите его в PyPI или используйте
#  командную строку для создания бинарного дистрибутива (исполняемого установщика).
#  Чтобы создать исходный дистрибутив, используйте командную строку, чтобы перейти в каталог,
#  содержащий setup.py, и выполните команду python setup.py sdist .
#  Запустите python setup.py bdist или, для Windows, python setup.py bdist_wininst, чтобы создать двоичный дистрибутив.
#  Используйте python setup.py register, а затем python setup.py sdist upload для загрузки пакета.
#  Наконец, установите пакет с помощью python setup.py install.

# TODO: Packaging
#  В предыдущем уроке были рассмотрены модули упаковки для использования другими программистами Python.
#  Однако у многих пользователей компьютеров, не являющихся программистами, Python не установлен.
#  Поэтому полезно упаковывать сценарии в виде исполняемых файлов для соответствующей платформы,
#  такой как операционные системы Windows или Mac. Это не обязательно для Linux, так как у большинства
#  пользователей Linux установлен Python, и они могут запускать сценарии как есть.
#  Для Windows доступно множество инструментов для преобразования скриптов в исполняемые файлы.
#  Например, py2exe можно использовать для упаковки скрипта Python вместе с необходимыми библиотеками в
#  один исполняемый файл. PyInstaller и cx_Freeze служат той же цели.
#  Для Mac используйте py2app, PyInstaller или cx_Freeze .
