...
# TODO: Precision (Четкость)
#  Двумя широко используемыми показателями для классификации являются precision (четкость) и recall (полнота).
...
# TODO: Precision (четкость) — это мера того, насколько точна модель с ее положительными предсказаниями

# TODO: Recall (Отзыв, Отзывать, Припоминание, Полнота)
...
# TODO: Precision & Recall Trade-off (Компромисс четкости и отзыва)
...
# TODO: Accuracy, Precision, Recall & F1 Score in Sklearn (Точность, Четкость, полнота и Оценка F1 в Sklearn)
...
#  Precision составляет 77%, что, как мы помним, является процентом правильных положительных прогнозов модели.
#  Recall составляет 69%, что является процентом положительных случаев, которые модель предсказала правильно.
#  F1 Score составляет 73%, что является средним значением точности и отзыва.
...
#  Мы можем изменить размер нашего тренировочного набора, используя параметр train_size.
...
# TODO: Наши значения accuracy, precision, recall и оценки F1 на самом деле очень похожи на значения,
...
#  Это связано с тем, что разделение тестов поезда выполняется случайным образом,
#  и в зависимости от того, какие точки попадут в тренировочный набор и тест, оценки будут разными.
...
#  Если вы помните, в Уроке 2 мы говорили о компромиссе между precision и recall.
...
#  но наши положительные прогнозы с большей вероятностью окажутся правильными. Это означает, что precision
#  будет выше, а recall ниже. С другой стороны, если мы снизим порог, у нас будет больше положительных прогнозов,
#  поэтому мы с большей вероятностью поймаем все положительные случаи.
#  Это означает, что recall будет выше, а precision ниже.
...
#  Эти значения демонстрируют тот же компромисс, что и precision и recall.
...
#  Можно построить кривую точного отзыва, но обычно это не делается.
...
# # 0.9214285714285714
# TODO: Обратите внимание, что в примере кода мы используем random_state в разделении тестов поезда,
