...
# TODO: What's a Neuron? (Что такое нейрон?)
#  Искусственный нейрон (часто называемый узлом) моделируется по образцу биологического нейрона.
#  Это простой объект, который может принимать входные данные, выполнять некоторые вычисления
#  с входными данными и производить выходные данные.
#  Визуально изобразим нейроны следующим образом. x1 и x2 являются входными данными.
#  Внутри нейрона выполняются некоторые вычисления на основе x1 и x2 для получения результата y1.
#  Нейроны могут принимать любое количество входных данных и также могут производить любое количество выходных данных.
#      См. Рис: WhatNeuronPicture
#  Каждый нейрон способен выполнять только небольшие вычисления,
#  но при совместной работе они становятся способными решать большие и сложные задачи.

# TODO: Neuron Computations (Нейронные вычисления)
#  Внутри нейрона, чтобы выполнить вычисления для получения выходных данных,
#  мы сначала помещаем входные данные в следующее уравнение (точно так же, как в логистической регрессии).
#      См. Рис: NeuronComputationsPicture_1.png
#  Напомним, что x1 и x2 являются входными данными.
#  В логистической регрессии мы называли значения w1, w2 и b коэффициентами.
#  В нейронных сетях мы называем w1 и w2 весами , а b — смещением, наклоном.
#  Мы подключаем это значение к так называемой функции активации.
#  Приведенное выше уравнение может иметь результат любого действительного числа.
#  Функция активации сжимает его в фиксированный диапазон (часто от 0 до 1).
#  Обычно используемой функцией активации является сигмовидная функция,
#  та же самая функция, которую мы использовали в логистической регрессии.
#  Напомним, что эта функция возвращает значение от 0 до 1.
#  Она определяется следующим образом.
#      См. Рис: NeuronComputationsPicture_2.png
#  Сигма имеет следующую форму.
#      См. Рис: NeuronComputationsPicture_3.png
#  Чтобы получить результат из входных данных, мы делаем следующее вычисление.
#  Веса, w1 и w2, и смещение, b, управляют тем, что делает нейрон.
#  Мы называем эти значения (w1, w2, b) параметрами.
#  Функция f является функцией активации (в данном случае сигмовидной функцией).
#  Значение y является выходом нейрона.
#      См. Рис: NeuronComputationsPicture_4.png
#  Эта функция может быть обобщена, чтобы иметь любое количество входных данных (xi) и,
#  следовательно, соответствующее количество весов (wi).

# TODO: Activation Functions (Функции активации)
#  Есть три часто используемые функции активации: сигмовидная (из предыдущей части), tanh и ReLU.
#  Tanh имеет форму, аналогичную сигмовидной, но находится в диапазоне от -1 до 1, а не от 0 до 1.
#  Tanh является гиперболической функцией тангенса и определяется следующим образом:
#      См. Рис: ActivationFunctionsPicture_1
#  График выглядит следующим образом:
#      См. Рис: ActivationFunctionsPicture_2
#  ReLU означает Rectified Linear Unit (Выпрямленная линейная единица).
#  Это функция идентичности для положительных чисел, которая переводит отрицательные числа в 0.
#  Вот уравнение и график.
#      См. Рис: ActivationFunctionsPicture_3
#      См. Рис: ActivationFunctionsPicture_4
# TODO: Любая из этих функций активации будет работать хорошо.
#  Какой из них использовать, будет зависеть от специфики наших данных.
#  На практике мы выясняем, какой из них использовать, сравнивая производительность разных нейронных сетей.

# TODO: An Example (Пример)
#  Предположим, у нас есть нейрон, который принимает 2 входа и производит 1 выход,
#  функция активации которого представляет собой сигмоиду. Параметры:
#      Weights (w1, w2) = [0, 1]
#      Bias (b) = 2
#  Если мы дадим нейрону ввод (1, 2), мы получим следующий расчет:
#      См. Рис: AnExamplePicture_1.png
# w1, w2 = 0, 1
# x1, x2 = 1, 2
# b = 2
# f = w1 * x1 + w2 * x2 + b
...
# print(round(y, 4))
# print(f'f({f})')
# TODO: Нейрон дает результат 0.9820.
#  В качестве альтернативы, если мы дадим нейрону ввод (2, -2), мы получим следующий расчет:
#      См. Рис: AnExamplePicture_2.png
# from math import e
#
# w1, w2 = 0, 1
# x1, x2 = 2, -2
...
# print(round(y, 4))
# print(f'f({f})')
# TODO: Нейрон с этим входом дает выход 0.5.
#  Нейрон сам по себе не обладает большой силой, но когда мы создаем сеть нейронов,
#  мы можем увидеть, насколько они сильны вместе.
# from math import e
#
# w1, w2 = 0, 1
# x1, x2 = 2, 1
...
# print(round(y, 4))
# print(f'f({f})')
# TODO: Multi-Layer Perceptron (Многослойный персептрон (восприятие, осознание))
#  Чтобы создать нейронную сеть, мы объединяем нейроны вместе, чтобы выходы одних нейронов были входами других нейронов.
#  Мы будем работать с нейронными сетями с прямой связью, что означает,
#  что нейроны посылают сигналы только в одном направлении.
#  В частности, мы будем работать с так называемым многослойным персептроном (MLP).
#  Нейронная сеть имеет несколько слоев, которые мы видим ниже.
#      См. Рис: MultiLayerPerceptronPicture.jpg
#  Многослойный персептрон всегда будет иметь один входной слой с нейроном (или узлом) для каждого входа.
#  В приведенной выше нейронной сети есть два входа и, следовательно, два входных узла.
#  Он будет иметь один выходной слой, с узлом для каждого выхода.
#  Выше есть 1 выходной узел для одного выходного значения.
#  Он может иметь любое количество скрытых слоев, и каждый скрытый слой может иметь любое количество узлов.
#  Выше есть один скрытый слой с 5 узлами. Узлы входного слоя принимают одно входное значение и передают его вперед.
#  Узлы в скрытых слоях, а также в выходном слое могут принимать несколько входных данных,
#  но они всегда производят один выходной сигнал. Иногда узлам необходимо передать свой вывод нескольким узлам.
#  В приведенном выше примере узлы входного слоя передают свои выходные данные каждому из пяти узлов скрытого слоя.
#  Однослойный персептрон — это нейронная сеть без скрытых слоев. Они редко используются.
#  Большинство нейронных сетей представляют собой многослойные персептроны,
#  как правило, с одним или двумя скрытыми слоями.

# TODO: Example Neural Network (Пример нейронной сети)
#  Давайте углубимся в то, как это работает, на примере. Нейронная сеть, которая решает любую реальную проблему,
#  будет слишком большой для интерпретации, поэтому мы рассмотрим простой пример.
#  У нас есть нейронная сеть с двумя входами, одним скрытым слоем с двумя узлами и одним выходом.
#  Веса и смещения приведены в узлах ниже. Все узлы используют сигмовидную функцию активации.
#      См. Рис: ExampleNeuralNetworkPicture_1.jpg
# TODO: Давайте посмотрим, какой результат мы получим для ввода (3, 2).
#  Вот вывод для первого узла в скрытом слое.
#      См. Рис: ExampleNeuralNetworkPicture_2.jpg
# from math import e
#
# w1, w2 = 0, 1
# x1, x2 = 3, 2
...
# print(f'f({f})')
# print(round(y, 4))
# TODO: Вот вывод для второго узла в скрытом слое.
#      См. Рис: ExampleNeuralNetworkPicture_3.jpg
# from math import e
#
# w1, w2 = 1, 2
# x1, x2 = 3, 2
...
# print(f'f({f})')
# print(round(y, 4))
# TODO: Вот вывод из узла в выходном слое.
#  Обратите внимание, что этот узел принимает выходные данные из скрытого слоя в качестве входных данных.
#      См. Рис: ExampleNeuralNetworkPicture_4.jpg
# from math import e
#
# w1, w2 = 1, -1
# x1, x2 = 0.8808, 0.9975
...
# print(f'f({f})')
# print(round(y, 4))
# TODO: Таким образом, для входа (3, 2) выход нейронной сети равен 0.8680.
#  Чтобы изменить работу нейронной сети, мы можем изменить веса и значения смещения.

# TODO: More Than 2 Target Values (Более 2 целевых значений)
#  Приятным преимуществом классификатора MLP является то, что он легко распространяется на задачи,
#  имеющие более двух целевых значений. В предыдущих модулях мы имели дело с предсказанием 0
#  или 1 (истина или ложь, выжил или нет, рак или нет).
#  В некоторых случаях мы будем выбирать из 3 или более возможных выходов.
#  Нейронная сеть делает это естественным образом. Нам просто нужно добавить больше узлов в выходной слой.
#  Например, если мы пытаемся предсказать, является ли изображение птицей, кошкой или собакой,
#  у нас будет три выходных узла. Первый (y1) измеряет, является ли изображение птицей,
#  второй (y2) измеряет, является ли изображение кошкой, а третий (y3) измеряет, является ли изображение собакой.
#  Модель выбирает выход с наибольшим значением.
#      См. Рис: MoreThan2TargetValuesPicture.jpg
#  Например, для заданного входного изображения нейронная сеть выдает y1=0.3, y2=0.2 и y3=0.5,
#  после чего модель определяет, что на изображении есть собака (y3).
#  Мы можем использовать любой классификатор для задачи с несколькими классами,
#  но нейронные сети естественным образом обобщаются.

# TODO: Loss (Потеря)
#  Чтобы обучить нейронную сеть, нам нужно определить функцию потерь.
#  Это мера того, насколько наша нейронная сеть далека от совершенства.
#  Когда мы обучаем нейронную сеть, мы оптимизируем функцию потерь.
#  Мы будем использовать перекрестную энтропию в качестве нашей функции потерь.
#  Это то же самое, что и вероятность, которую мы использовали в логистической регрессии,
#  но в данном контексте она называется по-другому.
#  Мы вычисляем перекрестную энтропию следующим образом.
#      См. Рис: LossPicture_1.png
#  Мы умножаем значения перекрестной энтропии для всех точек данных.
#  Допустим, у нас есть две модели для сравнения в крошечном наборе данных с 4 точками данных.
#  Вот таблица истинных значений, предсказанных вероятностей для модели 1 и предсказанных вероятностей для модели 2.
#      См. Рис: LossPicture_2.png
#  Перекрестная энтропия для модели 1 выглядит следующим образом.
#      См. Рис: LossPicture_3.png
# entropy_model_1 = 0.6 * 0.8 * (1 - 0.3) * (1 - 0.4)
# print(round(entropy_model_1, 4))
# TODO: Перекрестная энтропия для модели 2 выглядит следующим образом.
#      См. Рис: LossPicture_4.png
# entropy_model_2 = 0.5 * 0.9 * (1 - 0.1) * (1 - 0.5)
# print(round(entropy_model_2, 4))
# TODO: Перекрестная энтропия будет тем выше, чем лучше модель, поскольку модель 2 имеет более высокую
#  перекрестную энтропию, чем модель 1, она является лучшей моделью.
#  Как и в случае с функцией правдоподобия в логистической регрессии, мы используем функцию потерь,
#  чтобы найти наилучшую возможную модель.
# entropy_model = 0.6 * 0.3 * (1 - 0.6) * (1 - 0.8)
# print(round(entropy_model, 4))

# TODO: Backpropagation (Обратное распространение)
#  Нейронная сеть имеет множество параметров, которыми мы можем управлять.
#  Для каждого узла несколько коэффициентов, а узлов может быть очень много!
#  Процесс обновления этих значений для сходимости к наилучшей возможной модели довольно сложен.
#  Нейронная сеть работает в обратном направлении от выходного узла, итеративно обновляя коэффициенты узлов.
#  Этот процесс движения назад по нейронной сети называется Backpropagation или backprop.
#  Мы не будем вдаваться в подробности здесь, так как это связано с вычислением частных производных,
#  но идея состоит в том, что мы инициализируем все значения коэффициентов и итеративно изменяем значения,
#  чтобы на каждой итерации мы видели улучшение функции потерь.
#  В конце концов, мы больше не можем улучшать функцию потерь, и тогда мы нашли нашу оптимальную модель.
#  Прежде чем создавать нейронную сеть, мы фиксируем количество узлов и количество слоев.
#  Затем мы используем обратное распространение для итеративного обновления всех значений коэффициентов,
#  пока не сойдемся на оптимальной нейронной сети.

# TODO: Creating Artificial Dataset (Создание искусственного набора данных)
#  Иногда для проверки моделей полезно создать искусственный набор данных.
#  Мы можем создать набор данных необходимого размера и сложности.
#  Таким образом, мы можем создать набор данных, с которым легче работать, чем с набором данных из реальной жизни.
#  Это может помочь нам понять, как работают модели, прежде чем мы применим их к беспорядочным данным реального мира.
#  Мы будем использовать функцию make_classification в scikit-learn.
#  Он генерирует матрицу признаков X и целевой массив y.
#  Мы зададим ему такие параметры:
#      • n_samples: количество точек данных (по умолчанию n_samples=100)
#      • n_features: количество характеристик
#      • n_informative: количество информативных характеристик
#      • n_redundant: количество избыточных характеристик
#      • random_state: случайное состояние, чтобы каждый раз гарантировать один и тот же результат.
#  Вы можете просмотреть полную документацию, чтобы увидеть другие параметры,
#  которые вы можете настроить, чтобы изменить результат.
...
# print(X)
# print(y)
# TODO: Вот код для построения данных, чтобы мы могли посмотреть на них визуально:
# from matplotlib import pyplot as plt
#
# plt.scatter(X[y == 0][:, 0], X[y == 0][:, 1], s=100, edgecolors='k')
# plt.scatter(X[y == 1][:, 0], X[y == 1][:, 1], s=100, edgecolors='k', marker='^')
# plt.show()
# TODO: См. Рис: CreatingArtificialDatasetPicture.jpg
# TODO: Помимо make_classification, в Scikit-learn есть несколько других функций
#  для создания наборов данных классификации с различными свойствами.
#  Посмотрите на make_circles и make_moons, если вы хотите поиграть с более искусственными наборами данных.

# TODO: MLPClassifier (MLPКлассификатор)
#  В Scikit-learn есть класс MLPClassifier, который представляет собой многоуровневый персептрон для классификации.
#  Мы можем импортировать класс из scikit-learn, создать объект MLPClassifier и использовать метод fit для обучения.
#  Попробуй это сейчас:
# from sklearn.model_selection import train_test_split
...
# TODO: Вы заметите, что мы получаем ConvergenceWarning.
#  Это означает, что нейронной сети требуется больше итераций, чтобы сойтись на оптимальных коэффициентах.
#  Количество итераций по умолчанию — 200. Увеличим это значение до 1000.
#  mlp = MLPClassifier(max_iter=1000)
# TODO: Теперь, когда мы запустим этот код, нейронная сеть сойдется.
#  Теперь мы можем использовать метод оценки для расчета точности на тестовом наборе.
#  Попробуй это сейчас:
# from sklearn.model_selection import train_test_split
# from sklearn.neural_network import MLPClassifier
# from sklearn.datasets import make_classification
...
# TODO: Нейронные сети невероятно сложны, но scikit-learn делает их очень доступными в использовании!

# TODO: Parameters for MLPClassifier (Параметры для MLPClassifier)
#  Есть пара параметров, которые вам может понадобиться изменить в MLPClassifier.
#  Вы можете настроить количество скрытых слоев и количество узлов в каждом слое.
#  Классификатор MLPClassifier по умолчанию будет иметь один скрытый слой из 100 узлов.
#  Часто это работает очень хорошо, но мы можем поэкспериментировать с другими значениями.
#  Это создаст MLPCLassifier с двумя скрытыми слоями, одним из 100 узлов и одним из 50 узлов.
#      mlp = MLPClassifier(max_iter=1000, hidden_layer_sizes=(100, 50))
#  Мы видели max_iter в предыдущей части. Это количество итераций.
#  В общем, чем больше у вас данных, тем меньше итераций вам нужно для сходимости.
#  Если значение слишком велико, выполнение кода займет слишком много времени.
#  Если значение слишком мало, нейронная сеть не сойдется к оптимальному решению.
#  Нам также иногда нужно изменить альфу, которая является размером шага.
#  Это то, насколько нейронная сеть меняет коэффициенты на каждой итерации.
#  Если значение слишком мало, вы можете никогда не прийти к оптимальному решению.
#  Если значение слишком велико, вы можете пропустить оптимальное решение.
#  Первоначально вы можете оставить это значение по умолчанию.
#  Значение альфа по умолчанию равно 0.0001.
#  Обратите внимание, что уменьшение альфы часто требует увеличения max_iter
#  Иногда вам может понадобиться изменить решатель.
#  Именно такой алгоритм используется для поиска оптимального решения.
#  Все решатели будут работать, но вы можете обнаружить,
#  что для вашего набора данных другой решатель находит оптимальное решение быстрее.
#  Опции для решателя: «lbfgs», «sgd» и «adam».
#  Запустите этот код на игровой площадке и попробуйте изменить параметры MLPClassifier.
#  Код использует random_state, чтобы каждый раз, когда вы запускаете код с одними и теми же параметрами,
#  вы получали один и тот же результат.
# from sklearn.model_selection import train_test_split
# from sklearn.neural_network import MLPClassifier
# from sklearn.datasets import make_classification
#
...
# mlp.fit(X_train, y_train)
# print("accuracy:", mlp.score(X_test, y_test))
# TODO: Если посмотреть документацию, то можно прочитать еще о нескольких параметрах,
#  которые можно настроить в нейросети.
#  https://scikit-learn.org/stable/modules/generated/sklearn.neural_network.MLPClassifier.html

# TODO: The MNIST Dataset (Набор данных MNIST)
#  На этом уроке мы будем работать с новым набором данных, базой данных рукописных цифр MNIST.
#  NIST — это Национальный институт стандартов и технологий, а буква M означает «модифицированный».
#  Это база данных изображений рукописных цифр.
#  Мы построим классификатор, чтобы определить, какая цифра находится на изображении.
#  Мы начнем с версии набора данных MNIST, встроенной в scikit-learn.
#  Это изображения размером всего 8 на 8 пикселей, поэтому они размыты.
#  Вот пара примеров изображений:
#      См. Рис: TheMNISTDatasetPicture.jpg
# TODO: В scikit-learn мы можем загрузить набор данных с помощью функции load_digits.
#  Чтобы упростить задачу, мы изначально будем работать только с двумя цифрами (0 и 1),
#  поэтому мы используем параметр n_class, чтобы ограничить количество целевых значений до 2.
# from sklearn.datasets import load_digits
#
# X, y = load_digits(n_class=2, return_X_y=True)
# TODO: Мы можем увидеть размеры X и y и то, как выглядят значения, следующим образом.
...
#      0
# TODO: Мы видим, что у нас есть 360 точек данных, и каждая точка данных имеет 64 характеристики.
#  У нас есть 64 характеристики, потому что размер изображения 8 x 8 пикселей, и у нас есть 1 характеристика на пиксель.
#  Значение находится в шкале серого, где 0 — черный, а 16 — белый.
#  Чтобы получить более интуитивное представление о точке данных, измените форму массива на 8x8.
...
#  [ 0.  2. 14.  5. 10. 12.  0.  0.]
#  [ 0.  0.  6. 13. 10.  0.  0.  0.]]
#  Мы видим, что это 0, хотя в следующей части мы увидим, что можем нарисовать изображение более четко.
#  Попробуй это сейчас:
# from sklearn.datasets import load_digits
#
# X, y = load_digits(n_class=2, return_X_y=True)
...
# print(y[0])
# print(X[0].reshape(8, 8))
# TODO: Существуют разные версии этого набора данных с большим количеством пикселей и цветами (не в оттенках серого).
#  Мы увидим, что даже с этими упрощенными изображениями мы можем построить хороший классификатор.

# TODO: Drawing the Digits (Рисование цифр)
#  Вы можете построить модель, даже не глядя на визуальное представление изображений,
#  но иногда может быть полезно нарисовать изображение.
#  Мы используем функцию matplotlib matshow для рисования изображения.
#  Параметр cmap используется для указания того, что изображение должно быть в оттенках серого, а не в цвете.
# import matplotlib.pyplot as plt
# from sklearn.datasets import load_digits
#
# X, y = load_digits(n_class=2, return_X_y=True)
...
# # plt.xticks(())  # remove x tick marks (убрать х метки)
# # plt.yticks(())  # remove y tick marks (убрать y метки)
# plt.show()
# TODO: Выход:
#      См. Рис: DrawingDigitsPicture.jpg
#  Вы можете видеть, что всего с 64 пикселями изображение очень пикселизированное.
#  Даже с этими размытыми изображениями мы можем построить отличную модель.

# TODO: MLP for MNIST Dataset (MLP для набора данных MNIST)
#  Теперь давайте воспользуемся MLPClassifier для построения модели для набора данных MNIST.
#  Мы сделаем разделение обучения/тестирования и обучим MLPClassifier на тренировочном наборе.
# import matplotlib.pyplot as plt
# from sklearn.datasets import load_digits
# from sklearn.model_selection import train_test_split
# from sklearn.neural_network import MLPClassifier
# from sklearn.datasets import make_classification
#
# X, y = make_classification(n_features=64, n_redundant=0, n_informative=64, random_state=3)
# X_train, X_test, y_train, y_test = train_test_split(X, y, random_state=2)
# mlp = MLPClassifier()
# mlp.fit(X_train, y_train)
# TODO: Мы не получаем предупреждения, поэтому в данном случае достаточно количества итераций по умолчанию.
#  Давайте посмотрим, как модель предсказывает первую точку данных в тестовом наборе.
#  Мы используем matplotlib для рисования изображений, а затем показываем прогноз модели.
# TODO: Output:
#      См. Рис: MLPMNISTDatasetPicture_1.png
# x = X_test[0]
...
# print(mlp.predict([x]))
# # 0
# TODO: Мы видим, что это 0 и что наша модель правильно предсказывает 0.
#  Точно так же давайте посмотрим на вторую точку данных.
# x = X_test[1]
# plt.matshow(x.reshape(8, 8), cmap=plt.cm.gray)
# plt.xticks(())
# plt.yticks(())
...
# # 1
# TODO: Output:
#      См. Рис: MLPMNISTDatasetPicture_2.png
# TODO: Это явно 1, и наша модель снова получает правильный прогноз.
#  Мы также можем видеть точность модели на всем тестовом наборе.
# import time
#
# start = time.time()
# import matplotlib.pyplot as plt
...
# # 1
#
# print("Score is %s" % (mlp.score(X_test, y_test)))
# print("===%s seconds===" % (time.time() - start))
# TODO: Мы также можем видеть точность модели на всем тестовом наборе.
#  Таким образом, наша модель получает 100% точность.
#  0 и 1 — две цифры, которые легче всего различить, но мы увидим,
#  что модель также может хорошо работать с различением более сложных примеров.
...
# print(f"And I'm {mlp.score(X_test, y_test) * 100}% sure that I'm right.")
#
# plt.savefig('prediction.png')

# TODO: Classifying all 10 Digits (Классификация всех 10 цифр)
#  Поскольку нейронные сети легко обобщаются для обработки нескольких выходных данных,
#  мы можем просто использовать тот же код для создания классификатора, который различает все десять цифр.
#  На этот раз, когда мы загружаем цифры, мы не ограничиваем количество классов.
# from sklearn.datasets import load_digits
# from sklearn.model_selection import train_test_split
# from sklearn.neural_network import MLPClassifier
# import matplotlib.pyplot as plt
...
# TODO: Таким образом, мы получили 96% точек данных в тестовом наборе правильно.
#  Давайте посмотрим на те, которые мы получили неправильно.
#  Мы используем пустую маску, чтобы вытащить только те точки данных, которые оказались неверными.
#  Мы извлекаем значения x, истинное значение y, а также прогнозируемое значение.
# y_pred = mlp.predict(X_test)
# incorrect = X_test[y_pred != y_test]
# incorrect_true = y_test[y_pred != y_test]
# incorrect_pred = y_pred[y_pred != y_test]
# TODO: Давайте посмотрим на первое изображение, в котором мы ошиблись, и каков был наш прогноз.
# j = 0
...
# TODO: Вывод:
# TODO: истинное значение: 4
#  прогнозируемое значение: 9 Глядя на изображение, вы можете видеть, что человек тоже может быть сбит с толку.
#  Не очевидно, 4 это или 9. Попробуйте сейчас:
# from sklearn.datasets import load_digits
# from sklearn.model_selection import train_test_split
# from sklearn.neural_network import MLPClassifier
# import matplotlib.pyplot as plt
...
# plt.yticks(())
# plt.show()
# TODO: Вы можете изменить код, чтобы увидеть все точки данных, которые модель предсказала неправильно.

# TODO: Open ML (Открыть машинное обучение)
#  Для этого урока мы будем использовать более детализированную версию набора данных MNIST.
#  Вместо использования версии scikit-learn с 64-пиксельными изображениями
#  мы будем использовать версию OpenML с 784 пикселями (28 x 28).
#  Open ML (https://www.openml.org/) имеет базу данных больших наборов данных,
#  которые можно использовать для решения различных задач машинного обучения.
#  В Scikit-learn есть функция fetch_openml для прямой загрузки наборов данных из базы данных Open ML.
#  Используйте следующий код, чтобы получить наш набор данных.
# from sklearn.datasets import fetch_openml
# import numpy as np
# from sklearn.neural_network import MLPClassifier
#
# X, y = fetch_openml('mnist_784', version=1, return_X_y=True)
# TODO: Мы можем кратко взглянуть на форму массивов, диапазон значений признаков
#  и первые несколько значений целевого массива, чтобы лучше понять набор данных.
# print(X.shape, y.shape)
...
#      255.0
#      ['5' '0' '4' '1' '9']
# TODO: Мы видим, что у нас есть 70 000 точек данных с 784 характеристиками.
#  Значения признаков варьируются от 0 до 255 (которые мы интерпретируем по шкале серого,
#  где 0 соответствует белому цвету, а 255 — черному). Целевыми значениями являются числа 0-9.
#  Обратите внимание, что целевые значения хранятся в виде строк, а не целых чисел.
#  В нашем примере мы будем использовать только цифры 0-3,
#  поэтому мы можем использовать следующий код для сегментации этой части набора данных.
# X5 = X[y <= '3']
# y5 = y[y <= '3']
# TODO: Мы изменим некоторые параметры по умолчанию в MLPClassifier для построения модели.
#  Поскольку нашей целью будет визуализация весов скрытого слоя, мы будем использовать только 6 узлов в скрытом слое,
#  чтобы мы могли просмотреть их все. Мы будем использовать «sgd» (стохастический градиентный спуск)
#  в качестве нашего решателя, который требует от нас уменьшения альфа (скорости обучения).
# mlp = MLPClassifier(hidden_layer_sizes=(6,), max_iter=200, alpha=1e-4, solver='sgd', random_state=2)
#
# mlp.fit(X5, y5)
# TODO: Если мы запустим этот код, мы увидим, что он сходится.
#  Поскольку этот набор данных довольно большой, вы захотите работать с ним на своем компьютере,
#  а не на игровой площадке кода.
# from sklearn.datasets import fetch_openml
# from sklearn.neural_network import MLPClassifier
# import matplotlib.pyplot as plt
# import numpy as np
...
#
# print(X.shape, y.shape)
# # получить минимальное и максимальное значение пикселя first_image
# first_image = X.iloc[0]
# print(np.min(first_image), np.max(first_image))
# # вам нужно преобразовать y из категориального типа в тип int
# y = y.astype('int')
# print(y[0:5])
#
# X5 = X[y <= 3]
...
#
# print(mlp.coefs_[1].shape)  # (6, 4) –> 6 скрытые нейроны и 4 выходных нейрона
#
...
# TODO: MLPClassifier Coefficients (Коэффициенты MLP-классификатора)
#  MLPClassifier сохраняет коэффициенты в атрибуте coefs_.
...
#         -0.02957163],
# ...
# TODO: Сначала мы видим, что это список из двух элементов.
# print(len(mlp.coefs_))
# TODO: Выход:
# 2
# TODO: Два элемента в списке соответствуют двум слоям: скрытому слою и выходному слою.
#  У нас есть массив коэффициентов для каждого из этих слоев.
#  Давайте посмотрим на форму коэффициентов для скрытого слоя.
# print(mlp.coefs_[0].shape)
# Output:
# (784, 6)
# TODO: Мы видим, что у нас есть двумерный массив размером 784 x 6. Есть 6 узлов и 784 входных значения,
#  поступающих в каждый узел, и у нас есть вес для каждого из этих соединений.
#  Чтобы интерпретировать значения, нам нужно будет использовать визуальное представление.

# TODO: Visualizing the Hidden Layer (Визуализация скрытого слоя)
#  Чтобы лучше понять, что делает нейронная сеть, мы можем визуализировать веса скрытого слоя,
#  чтобы получить некоторое представление о том, что делает каждый узел.
#  Мы снова будем использовать функцию matshow из matplotlib для рисования изображений.
#  В matplotlib мы можем использовать функцию subplots для создания нескольких графиков на одном графике.
# from sklearn.datasets import fetch_openml
# from sklearn.neural_network import MLPClassifier
# import matplotlib.pyplot as plt
# import numpy as np
...
# plt.show()
# TODO: Вывод:
#      См. Рис: VisualizingHiddenLayerPicture.jpg
# TODO: вы можете видеть, что узлы 4 и 6 определяют, является ли цифра 3.
#  Узел 1 определяет, является ли цифра 0 или 2, поскольку вы можете видеть оба этих значения на изображении.
#  Не каждый скрытый узел будет иметь очевидное применение.
#  Если вы измените случайное состояние в MLPClassifier, вы, скорее всего, получите другие результаты.
#  Существует много эквивалентно оптимальных нейронных сетей, которые работают по-разному.

# TODO: Interpretability (Интерпретируемость)
#  Хотя мы можем визуализировать узлы в скрытом слое, чтобы понять на высоком уровне, что делает нейронная сеть,
#  невозможно ответить на вопрос «Почему точка данных x получила прогноз y?»
#  Поскольку существует так много узлов, каждый со своими коэффициентами,
#  невозможно получить простое объяснение того, что делает нейронная сеть.
#  Это затрудняет интерпретацию модели и ее использование в определенных бизнес-вариантах.
#  Нейронные сети не являются хорошим вариантом для интерпретируемости.

# TODO: Computation (Вычисление)
#  Обучение нейронных сетей может занять приличное количество времени.
#  Каждый узел имеет свои собственные коэффициенты, и для обучения они итеративно обновляются,
#  поэтому это может занять много времени. Однако они поддаются распараллеливанию,
#  поэтому на них можно направить вычислительную мощность, чтобы они обучались быстрее.
#  После того, как они построены, нейронные сети не спешат делать прогнозы,
#  однако они не так быстры, как некоторые другие модели.

# TODO: Performance (Производительность)
#  Главное достоинство нейронных сетей — их производительность.
#  По многим задачам их производительность просто не может быть лучше других моделей.
#  Они могут выполнить некоторую настройку параметров, чтобы найти оптимальную производительность,
#  но они выигрывают от необходимости минимального проектирования функций до построения модели.
#  Во многих более простых задачах вы можете добиться эквивалентной производительности с помощью более простой модели,
#  такой как логистическая регрессия, но с большими неструктурированными наборами
#  данных нейронные сети превосходят другие модели.
#  Ключевым преимуществом нейронных сетей является их производительность.

# TODO: ЗАДАЧА: Machine Learning - The Sigmoid Function (Машинное обучение — сигмовидная функция)
#  Рассчитать выход узла.
#  Задача Вам даны значения для w1, w2, b, x1 и x2, и вы должны вычислить результат для узла.
#  Используйте сигмовидную как функцию активации.
#  Формат ввода: w1, w2, b, x1 и x2 в одной строке, разделенной пробелами
#  Формат вывода: Число с плавающей запятой, округленное до 4 знаков после запятой
#      Sample Input:
#      0 1 2 1 2
#      Sample Output:
#      0.9820
#  См. Рис: MachineLearningSigmoidFunctionPicture.jpg
